...
int main(int argc, char** argv){
	...
	int64_t scan, temp;
	int64_t current_reflength, current_start, current_end, coverage_counter = 0;
	double coverage;
	...
	if (argc < 5)
			terror("USE: ./uniseqDBCoverage <Fixed_Parsed_Blast> <DB.taxo> <uniseqDB> <Output>");
	...
	printf("Reading Fixed_Parsed_Blast...\n");
	while(!feof(fi)){
	...
	FILE * fseq;
	if((fseq=fopen(strcat(argv[4],".seq"),"wt"))==NULL){
		terror("Can't open FSEQ file");
	}
	//fprintf(fseq, ">%s | L:%"PRIu64"\n", argv[4], DB_LENGTH);

	uint64_t max_hits_per_pixel = (uint64_t)(DB_LENGTH / (PNG_DIM * PNG_DIM)) + 1;
	uint64_t pixel_counter = 0, hits_at_pixel = 0;
	...
	for(ii = 0; ii<DB_LENGTH; ii++){
		...
		if((int)((uint64_t)(ii  + 1)  % (uint64_t) 70)== 0){
			fprintf(fseq, "\n");
		}*/
	}
	...
	printf("UniSeq Length\tCovered Bases\tCoverage Percentage\n%"PRId64"\t%"PRId64"\t%lf\n",
		DB_LENGTH, coverage_counter, coverage);
